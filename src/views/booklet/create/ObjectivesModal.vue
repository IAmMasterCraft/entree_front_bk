  ...
  <div>
    ...
    >
      ...
      <template v-slot:header>
        <h5 class="px-3">{{ checkAction }} Subject Objective</h5>
        <i
          class="btn btn-info fa fa-close pull-right"
        ...
        <CCol sm="6">
          <CInputCheckbox
            :label="`Objective Status (${
              formValues.status ? 'is achieved' : 'is NOT achieved'
            })`"
            required="true"
      ...
      objectives(newObjective) {
        //   console.log(newObjective);
          if (newObjective != null) {
              this.formValues = newObjective;
              this.formValues.status = (this.formValues.status.toLowerCase().includes("not")) ? false : true;
