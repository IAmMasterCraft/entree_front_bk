            ...
            <CCard class="text-center" @click="navigation({id:myClass.id, grade_name: myClass.grade_name,})">
                ...
                <CCardFooter>
                  <CBadge color="success"> 
                    {{ (!myClass.subject_count) ? 0 : myClass.subject_count }} {{ (myClass.subject_count && myClass.subject_count > 1) ? "Subjects" : "Subject" }} 
                  </CBadge>
                </CCardFooter>
            </CCard>
          </CCol>
...
export default {
  ...
  methods: {
    ...
    navigation(classProps) {
      this.$router.push({name: "Manage Subjects / ", params: {id: classProps.id, class: classProps.grade_name.toLowerCase()}});
    }, //end of navigation
  },
  created () {
    this.getClasses();
